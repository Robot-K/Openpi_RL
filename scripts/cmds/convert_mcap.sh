...
# ─── CONFIG ────────────────────────────────────────────────────────────────────
DATA_DIR=/data/kding/FastWAM/mcap_data/fold_clothv3    # mcap 数据目录（含 config.py），用绝对路径指向 FastWAM
...
HF_LEROBOT_HOME=./lerobot_data uv run --no-sync examples/airbot/convert_mcap_data_to_lerobot.py \
    --data_dir "${DATA_DIR}" \
    ${ARGS}
